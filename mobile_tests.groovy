...
timeout("1200") {
    node("maven") {
        ...
        stage('Running mobile tests via ansible') {
            def state = sh(
                    script: "ansible-playbook -i playbook/hosts playbook/tests.yaml --extra-vars appium_server_url=${yamlConfig['appium_server_url']} --extra-vars wiremock_url=${yamlConfig['wiremock_url']} --extra-vars jdbc_url=${yamlConfig['jdbc_url']}",
                    returnStatus: true
            )
            if (state > 0) {
